...
from zotmer.library.bits import ffs, rev, popcnt, m1
...
_fasIdx = dict([(_fas[i],i) for i in range(16)])
...
def unFasta(c):
    """Convert a character from the 16-letter FASTA alphabet
    to a 4-bit indicator set showing which bases are present.

    If the character is not one of the 16-letter FASTA alphabet
    the value None will be returned.
    """
    return _fasIdx.get(c, None)
...
def kmersWithPosList(k, seq, bothStrands=False):
    """
    Extract *k*-mers from a string nucleotide sequence `seq`.
    The parameter `bothStrands` determines whether the sequence of
    result *k*-mers should include the reverse complement of each *k*-mer
